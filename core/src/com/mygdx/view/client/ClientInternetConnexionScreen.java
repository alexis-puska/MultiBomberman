...
public class ClientInternetConnexionScreen implements Screen, MenuListener {
	...
	private List<Server> listOfServer;

	public ClientInternetConnexionScreen(final MultiBombermanGame mbGame) {
		this.mbGame = mbGame;
		this.cursorPos = 0;
	...
	private void refresh() {
		discovery.refreshInternetServerList();
		displayListOffset = 0;
	}

	@Override
	public void render(float delta) {
		listOfServer = discovery.getInternetServer();
		...
		mbGame.getBatch().begin();
		layout.setText(font, MessageService.getInstance().getMessage("game.menu.clientInternetConnexionScreen"));
		font.draw(mbGame.getBatch(), layout, (Constante.SCREEN_SIZE_X / 2) - (layout.width / 2), 340);

		layout.setText(font,
				MessageService.getInstance().getMessage("game.menu.clientInternetConnexionScreen.refresh"));
		font.draw(mbGame.getBatch(), layout, 50, 320);

		layout.setText(font,
				MessageService.getInstance().getMessage("game.menu.clientInternetConnexionScreen.server")
		...
		if (listOfServer != null) {
			...
			for (int i = displayListOffset; i < displayListOffset + NB_SERVER_DISPLAYED; i++) {
				if (i < listOfServer.size()) {
					Server s = listOfServer.get(i);
					layout.setText(font,
							s.getWanIp() + ":" + s.getPort() + " - " + s.getLookup().getPostal() + " "
									+ s.getLookup().getCityName() + " " + s.getLookup().getCountryName() + " " + "("
									+ s.getLookup().getState() + ")");
	...
	public void pressStart() {
		if (cursorPos == 0) {
			...
			Server dsi = listOfServer.get((cursorPos - 1) + displayListOffset);
			if (mbGame.getNetworkService().connectToServer(dsi.getPort(), dsi.getWanIp(), cvs)) {
